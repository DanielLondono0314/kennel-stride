...
| Fecha | Qué |
|-------|-----|
| 2026-07-07 (QA) | **Sesión QA integral en prod** — 4 bugs encontrados y ARREGLADOS+desplegados el mismo día: (1) **CRÍTICO** embed ambiguo `facility_units` → HTTP 300 → Dashboard/Solicitudes/Calendario vacíos EN SILENCIO desde 2026-06-04; (2) errores de fetch ahora visibles con retry (antes indistinguibles de "sin datos"); (3) deep-links perdidos en hard-load (race org-null en OrgGuard → rebote a /login → dashboard); (4) chunks purgados tras deploy → auto-recarga única en ErrorBoundary. Flujo del dinero verificado E2E en prod: solicitud→aprobar→check-in (perrera)→check-out con bono (crédito 5→4, perrera liberada, sin factura). Menores abiertos: "Esperados Hoy" no filtra por fecha (tab vs KPI inconsistentes); asterisco engañoso en Teléfono de cliente; perrera de kaelis con periodo vencido desde 5 jun (dato heredado del bug 1, decidir el usuario). |
