...
export function ConditionsTab({ dogId, dogName }: Props) {
  ...
  const handleSave = async () => {
    ...
    if (error) {
      ...
      setModalOpen(false);
      setForm({ condition_name: "", condition_type: "disease", diagnosed_date: new Date().toISOString().split("T")[0], status: "active", severity: "moderate", treatment: "", notes: "" });
      fetchRecords();
    }
  };
  ...
  const handleResolve = async (id: string) => {
    const { error } = await supabase
      .from("medical_conditions")
      .update({ status: "resolved", resolved_date: new Date().toISOString().split("T")[0], updated_at: new Date().toISOString() })
      .eq("id", id);
    if (!error) {
      toast.success("Condición marcada como resuelta");
      fetchRecords();
    }
  };
  ...
  return (
    <div className="mt-4 space-y-4">
              ...
              <CardContent className="p-4">
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <Badge className={statusMap[r.status]?.className || "bg-muted"}>
                      {statusMap[r.status]?.label || r.status}
                    </Badge>
                    <Badge className={severityMap[r.severity]?.className || "bg-muted"}>
      ...
      <Dialog open={modalOpen} onOpenChange={setModalOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Registrar Condición — {dogName}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              <Label>Condición / Enfermedad</Label>
              <Input value={form.condition_name} onChange={(e) => setForm({ ...form, condition_name: e.target.value })} placeholder="Nombre de la condición" />
            </div>
              ...
              <div>
                ...
                <Select value={form.condition_type} onValueChange={(v) => setForm({ ...form, condition_type: v })}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="congenital">Congénita</SelectItem>
              ...
              <div>
                ...
                <Select value={form.status} onValueChange={(v) => setForm({ ...form, status: v })}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="chronic">Crónica</SelectItem>
                    <SelectItem value="monitoring">En Observación</SelectItem>
            ...
            <div>
              <Label>Notas</Label>
              <Textarea value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} placeholder="Observaciones adicionales" />
            </div>
          </div>
          <DialogFooter>
