...
import { Plus, Bug, CheckCircle, AlertTriangle, Clock } from "lucide-react";
...
export function DewormingTab({ dogId, dogName }: Props) {
  ...
  const handleSave = async () => {
    ...
    const { error } = await supabase.from("deworming_records").insert({
      ...
      weight_at_time: form.weight_at_time ? parseFloat(form.weight_at_time) : null,
    ...
    if (error) {
      ...
      setModalOpen(false);
      setForm({ product_name: "", product_type: "internal", date_administered: new Date().toISOString().split("T")[0], next_dose_date: "", weight_at_time: "", veterinarian: "", notes: "" });
      fetchRecords();
    }
  };
  ...
  const getStatus = (r: any) => {
    ...
    if (isFuture(next) && next <= addDays(new Date(), 14))
      return { label: "Próxima", icon: Clock, className: "bg-warning/10 text-warning" };
  ...
  return (
    <div className="mt-4 space-y-4">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <Badge variant="outline" className="text-xs">
                        {r.product_type === "internal" ? "Interna" : r.product_type === "external" ? "Externa" : "Mixta"}
                      </Badge>
                    </div>
      ...
      <Dialog open={modalOpen} onOpenChange={setModalOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Registrar Desparasitación — {dogName}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              <Label>Producto</Label>
              <Input value={form.product_name} onChange={(e) => setForm({ ...form, product_name: e.target.value })} placeholder="Nombre del producto" />
            </div>
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <Label>Peso al momento (kg)</Label>
                <Input type="number" step="0.1" value={form.weight_at_time} onChange={(e) => setForm({ ...form, weight_at_time: e.target.value })} />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
              ...
              <div>
                ...
                <Input type="date" value={form.next_dose_date} onChange={(e) => setForm({ ...form, next_dose_date: e.target.value })} />
              </div>
            </div>
            <div>
